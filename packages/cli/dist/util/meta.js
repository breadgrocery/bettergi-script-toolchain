...
export const syncManifestConfig = async (outDir, config) => {
...
const validateManifest = (manifest) => {
    if (!manifest.name) {
        throw new Error("Missing manifest.name. Set `manifest.name` in bettergi.config or `name` in package.json.");
    }
    if (!manifest.version) {
        throw new Error("Missing manifest.version. Set `manifest.version` in bettergi.config or `version` in package.json.");
    }
    if (!manifest.description) {
        throw new Error("Missing manifest.description. Set `manifest.description` in bettergi.config or `description` in package.json.");
    }
    if (!manifest.authors || manifest.authors.filter((author) => author?.name).length === 0) {
        throw new Error("Missing manifest.authors. Set `manifest.authors` in bettergi.config or `author` in package.json.");
    }
};
export const syncSettingsConfig = async (outDir, config) => {
