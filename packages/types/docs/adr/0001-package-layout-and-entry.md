...
每个声明文件是外部模块：CLR 全限定名写在 type-only ambient namespace 中；真实运行时值只由 bindings 提供。依赖使用**无扩展名**相对路径直接 import——需要激活 ambient 图时用 side-effect `import "..."`，仅类型用 `import type`；文件末尾 `export {}`。不设 namespace barrel，不假设传递副作用。
