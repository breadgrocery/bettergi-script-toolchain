...
**脚本字符串域**:
有限已知字面量联合，通常带 `(string & {})` 开放回退；用于补全，不是运行时枚举；须 ambient re-export 供 FQN
_Avoid_: 任意 string、伪装枚举、替代 CLR 枚举 HostType
...
| 角色名称（中文）   | 中文角色名；类型常为 `AvatarName`            | 中文角色名称字段         |
| 角色英文名（配置） | JSON `nameEn`；类型常为 `CombatAvatarNameEn` | 与 YOLO class 混称       |
| 当前角色           | 策略出战位特殊名                             | 与真实角色名混写         |
| 元素战技           | E 技能                                       | 小技能（公开声明中）     |
| 元素爆发           | Q 技能                                       | 大招（公开声明中）       |
...
| 赶路模式           | `TravelMode`                                 | 与路径点 `MoveMode` 混淆 |
